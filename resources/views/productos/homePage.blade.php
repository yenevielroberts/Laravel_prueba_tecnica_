...
<body>
    <header class="bg-orange-200 w-screen h-20 flex items-center">
        <nav class="flex flex-row w-full items-center justify-between">
            ...
            <div class="flex flex-row w-full justify-end">
                <a class="m-3">Buscar</a>
                <a class="m-3">Perfil</a>
            </div>
        </nav>
    </header>
<main class="flex flex-col w-screen">
    <!--Section donde solo esta la imagen-->
    <section class="h-100 bg-cover md:h-150" style="background-image: url('/img/bgHomeSection.png')">
    </section>
    <!--Section productos destacos-->
    <section class="bg-teal-900 w-screen">
        ...
        <div class="flex flex-row flex-wrap justify-evenly  mt-5 mb-5">
            @foreach ($productos as $products )
                <div class="flex flex-col items-center">
                    <img src="/img/comida/pasta.png" class="w-55 h-70 rounded-xl">
                    <p class="text-white">{{ $products->nombre_pro }}</p>
                    <p class="rounded-full bg-lime-300 p-1 mt-2"> {{ $products->precio_pro}}€</p>
                </div>
            @endforeach
         </div>
    </section>
    <!--Section nuestros productos-->
    <section class="bg-orange-200 w-screen">
           <div class="flex flex-row items-center ml-5 "> 
...
<footer class="bg-teal-900 w-screen h-40 flex flex-row ">
